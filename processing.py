...
# create model
            model_hijack.undo_hijack(shared.sd_model)
...
#                    module = network_restore_weights_from_backup(module)
            ...
            temp_tensors = shared.sd_model.state_dict()

            for key in list(temp_tensors.keys()):
                if key in POPKEYS:
                    del temp_tensors[key]

            model_hijack.hijack(shared.sd_model)

            for key in list(temp_tensors.keys()):
                if "cond_stage_model." in key:
                    del temp_tensors[key]

#            temp_tensors = to_half(temp_tensors)
#            temp_tensors = prune_model(temp_tensors, True)

            for key in temp_tensors.keys():
#                v = copy.deepcopy(temp_tensors[key])
                v = temp_tensors[key]
                v = v.contiguous()
                temp_tensors[key] = v

            safetensors.torch.save_file( temp_tensors, "test_model.safetensors")
